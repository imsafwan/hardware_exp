...
uav_flying_time = 1.95*60 # min*60
...
def replan_with_rendezvous(uav_state, ugv_state, uav_actions, road_network):
    ...
    if actions_trim != []:
        ...
        for i in range(k, -1, -1):
            ...
            #print('candidate seq: ', candidate_seq)
...
def create_ugv_plan(ugv_state, new_rp, road_network):
    ...
    """
    start_node = "C" #ugv_state["next_road_point"]
    target_node = "C" #new_rp["node"]

    # shortest path on road network
    path = nx.shortest_path(road_network, start_node, target_node, weight="weight")
...
def find_new_rp(seq, uav_cur_state, candidate_rendezvous_points):
    ...
    #print('-------')
    #print('Candidate rendezvous points: ', candidate_rendezvous_points)
...
def compute_candidate_points(uav_state, ugv_state, road_network):
    ...
    for node in road_network.nodes:
        ...
        dist_to_ugv =  nx.shortest_path_length(road_network, "C", node, weight="weight")   # meters
...
uav_plan, status = replan_with_rendezvous(uav_state, ugv_state, uav_actions, road_network)

print("Replan status:", status)
if status != 'KEEP_OLD_RENDEZVOUS':
    
    ugv_plan = create_ugv_plan(ugv_state, uav_plan[-2]["location"], road_network)
    print("New plan:")
    print(uav_plan)
    print('*****************')
    print(ugv_plan)

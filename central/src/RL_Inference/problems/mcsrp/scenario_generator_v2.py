...
def scenario_gen(
    ...
    max_possible_points = int((ugv_area_size * grid_resolution) ** 2)
    
    if N_ugv > max_possible_points * 0.5:  # Safety margin
        raise ValueError(f"Requesting {N_ugv} UGV nodes is too many for area_size={ugv_area_size}. "
                        f"Maximum safe: ~{int(max_possible_points * 0.5)}. "
                        f"Increase ugv area_size or reduce ugv_graph_size.")
    ...
    for attempt in range(max_attempts):
        ...
        candidates = (np.random.rand(n_generate, 2) * ugv_area_size).astype(np.float32)
    ...
    if len(ugv_loc) < N_ugv:
        raise ValueError(f"Could not generate {N_ugv} unique UGV points after {max_attempts} attempts. "
                        ...
                        f"This area_size={ugv_area_size} is too small. "
                        f"Try: area_size >= {ugv_area_size * np.sqrt(N_ugv / len(ugv_loc)):.1f}")

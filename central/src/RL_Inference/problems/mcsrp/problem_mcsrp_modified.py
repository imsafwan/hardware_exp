...
def generate_instance(uav_graph_size, ugv_graph_size, k_neighbors=1, area_size=1.0, device='cuda'):

    ugv_loc, uav_loc, sp_matrix = scenario_gen(ugv_graph_size,uav_graph_size)

    # print(ugv_loc)
    # print(sp_matrix)
    # kk

    # 5️ Convert UAV and UGV locations to tensors
    uav_mission_points = uav_loc.to(device)
    ugv_mission_points = ugv_loc.to(device)
    unique_mission_points = torch.cat([ugv_mission_points,uav_mission_points], dim=0) 
    depot_ix = torch.randint(0, ugv_graph_size, (1,), device=device)

    # 6️ Concatenate features for encoder input
    ones = torch.ones(uav_mission_points.size(0), 1, device=device)
    zeros = torch.zeros(ugv_mission_points.size(0), 1, device=device)
    encoder_space = torch.cat([
        torch.cat([ugv_mission_points, zeros], dim=-1),
        torch.cat([uav_mission_points, ones], dim=-1)
    ], dim=0)


    # print(ugv_mission_points.shape)
    # print(uav_mission_points.shape)
    ...
    return {
        ...
        'sp_matrix': sp_matrix,   #  Shortest path distances along the graph
    }

...
class UAV_UGV_Env(gym.Env):
    ...
    def __init__(self):
         ...
         self.batch_size = 256
         self.uav_speed = 0.9    # m/sec constant
    ...
    def calculate_distances(self, p1, p2):
         ...
         return torch.norm((p1 - p2), dim=-1)* 1 # m #torch.sqrt(torch.sum((p1 - p2) ** 2, dim=-1))*1
    ...
    def step(self, actions, step):
        ...
        step_elapsed_times = (torch.norm((action_coords - self.uav_pos_coord), dim=1)*1 / self.uav_speed)
        self.uav_fuel = self.uav_fuel - (step_elapsed_times * 1).int() # fuel depletion
        ...
        recharge_time = torch.where(is_refuel_stop, torch.tensor(120).float().to(device), torch.zeros_like(action_indices).float().to(device))

        mask = (is_refuel_stop == True)
        
        
        
        #print((self.cu_dis1(self.refuel_stop, action_coords)/self.ugv_speed)[index_of_interest])
        self.refuel_stop_time[mask] =  self.refuel_stop_time[mask] + (self.cu_dis1(self.refuel_stop, action_coords)/self.ugv_speed)[mask]
        #print('self.refuel_stop_time --->', self.refuel_stop_time[index_of_interest])
        ...
        nonzero_indices = torch.where(waiting_time != 0)[0]  # indices of non-zero entries

        # if len(nonzero_indices) > 0:
        #     print(f" Waiting time non-zero at batches: {nonzero_indices.tolist()}")
        #     print("waiting_time values:", waiting_time[nonzero_indices])
        #     assert False, "Waiting time is not zero for some batches!"
        # else:
        #     print(" All waiting times are zero.")
        
        assert torch.all(waiting_time == 0), "Waiting time is not zero for some batches!"
        #print('waiting time --->', waiting_time[index_of_interest])

        step_elapsed_times = step_elapsed_times + recharge_time + waiting_time # accounts for recharging time
    ...
    def feasible_action(self):
       ...
       return_feasible_region = (((total_distance/ self.uav_speed)*1).int() <= (self.uav_fuel.unsqueeze(1)).int()).to(device) 
       ...
       # -------- UGV positions where UGV can reach before UAV -------- #
       ...
       #print('time_to_ugv_nodes_ugv --->', time_to_ugv_nodes_ugv[index_of_interest])

       #print('mission_time_elapsed --->', self.mission_time_elapsed[index_of_interest])

       # -------- Mask creation -------- #

       # Mask = True where UGV reaches earlier (i.e., UAV arrives later)
       ugv_reach_before_uav_mask = (
            (self.mission_time_elapsed.unsqueeze(1) + time_to_ugv_nodes_uav) > (self.refuel_stop_time.unsqueeze(1) + time_to_ugv_nodes_ugv)
        ).to(device)

       # Create a tensor of ones for UAV mission points [B, N_uav]
       uav_mask_padding = torch.ones(
            (ugv_reach_before_uav_mask.shape[0], len(self.uav_mission_points[0])),
            device=device,
            dtype=ugv_reach_before_uav_mask.dtype
        )

       # Concatenate along the node dimension
       ugv_reach_before_uav_mask = torch.cat(
            (ugv_reach_before_uav_mask, uav_mask_padding), dim=1
        ) # → [B, N_ugv + N_uav]
       ...
       feasible_mask[:,:] =  return_feasible_region*repeated_feasible_region*ugv_reach_before_uav_mask
       ...
       #print('ugv_reach_before_uav_mask  --->', ugv_reach_before_uav_mask[index_of_interest])
       ...
       if exceeded_indices.numel() > 0:
         ...
         feasible_mask[exceeded_indices, ugv_slice] = (
    return_feasible_region[exceeded_indices, ugv_slice].float() *
    ugv_reach_before_uav_mask[exceeded_indices, ugv_slice]
)

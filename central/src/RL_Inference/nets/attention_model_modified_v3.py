...
index_of_interest = 25 # what you want to print

def print_gpu_memory():
    GPUs = GPUtil.getGPUs()
    for i, gpu in enumerate(GPUs):
        print(f"GPU {i}: {gpu.memoryUsed}MB / {gpu.memoryTotal}MB ({gpu.memoryUtil*100}%)")
...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.is_mcsrp = problem.NAME == 'mcsrp'
        ...
        self.age_embed = nn.Linear(1, embedding_dim)
        self.graph_embed = nn.Linear(embedding_dim, embedding_dim)
    ...
    def forward(self, input, return_pi=False):
        ...
        if self.checkpoint_encoder and self.training:  # Only checkpoint if we need gradients
            ...
            all_mission_points[:,:,:2] = all_mission_points[:, :, :2] / 10
            
            #assert all_mission_points.size(1) == 80
            #cprint('All mission points number = {}'.format(all_mission_points.shape), 'yellow', attrs = ['bold'])
            st = time.time()
        ...
        st = time.time()
        _log_p, pi, cost, _ = self._inner(input, embeddings) #_log_p = [ B, number of action step, N]   #pi = [B, number of action step] #cost = [B]
    ...
    def _inner(self, input, embeddings):
        ...
        batch_size = env.batch_size #state.ids.size(0)
        outputs = []
        sequences = []
        agents = []
        ...
        while True: #True: #not (self.shrink_size is None and state.all_finished()):
        
        
            
            
            age_period = state[2].clone()        # [B, N_uav] binary: 0 (unvisited), 1 (visited)
            
            # Pass through embedding layer (still valid for binary)
            age_embeddings = self.age_embed(age_period[:, :, None])
            
            
            modi_embeddings = embeddings + age_embeddings
            
            fixed = self._precompute(modi_embeddings)
                
            
            #st = time.time()
            log_p, mask = self._get_log_p(modi_embeddings, fixed, state) 
            #print('mask time ------>', time.time()-st)
            #log_p = [ B, 1, 22(N)] #mask = [ B, 1, 22(N)]
            
            selected = self._select_node(log_p.exp()[:, 0, :], mask[:, 0, :])  # Squeeze out steps dimension # selected : [B]
           
            agent = env.agent.clone() #[B]
            

            state,reward,done,_ = env.step(selected, j) #state #.update(selected)
            
            if self.decode_type == "greedy":
                cprint('Action -------> {}, agent {}'.format(selected[index_of_interest], agent[index_of_interest]), 'red', attrs = ['bold'])
            else:
                cprint('Action -------> {}, agent {}'.format(selected[index_of_interest], agent[index_of_interest]), 'cyan', attrs = ['bold'])
            #print('Reward------>', reward[index_of_interest], 'cost ---->', cost[index_of_interest])
        ...
        cost_unified = cost.clone()
        
        

        # Collected lists, return Tensor
        return torch.stack(outputs, 1), torch.stack(sequences, 1), cost_unified, torch.stack(agents, 1)
    ...
    def sample_many(self, input, batch_rep=1, iter_rep=1):
        ...
        all_mission_points = self._init_embed(input).clone()
        print('All mission points shape = {}'.format(all_mission_points))
        all_mission_points[:,:,:2] = all_mission_points[:, :, :2] / 10
        
        return sample_many(
            lambda input: self._inner(*input),  # Need to unpack tuple into arguments
            lambda input, pi: self.problem.get_costs(input[0], pi),  # Don't need embeddings as input to get_costs
            (input, self.embedder1(all_mission_points)[0]),  # Pack input with embeddings (additional input)
            batch_rep, iter_rep
        )

    def _select_node(self, probs, mask):
        
        #print('Probability--->',probs[index_of_interest])
        #print('Mask--->',mask[index_of_interest])
    ...
    def _get_log_p(self,embeddings, fixed, state, normalize=True):
        ...
        h_g = embeddings.mean(1)[:, None, :]
        graph_mean = self.graph_embed(h_g)
        ...
        #print('Query ------>', query[index_of_interest])
        # print('Graph mean ------>', graph_mean[index_of_interest])
        # print('Step context ------>', self._get_parallel_step_context(fixed.node_embeddings, state)[index_of_interest])
        ...
        #print('Mask before------>', mask[index_of_interest])

        
        # Compute logits (unnormalized log_p)
        log_p, glimpse = self._one_to_many_logits(query, glimpse_K, glimpse_V, logit_K, mask)
        #print('Log p ------>', log_p[index_of_interest])
    ...
    def _get_parallel_step_context(self, embeddings, state):
        ...
        current_node =  state[0] #state.get_current_node() # current node indices[ B, 1]
        ...
        if self.is_vrp:
            ...
            uav_fuel = ((state[1]) / (env.uav_fuel_constant)).unsqueeze(1)

            # embedding of current UAV node
            current_embed = torch.gather(
                embeddings,
                1,
                current_node.view(batch_size, num_steps, 1)
                    .expand(batch_size, num_steps, embeddings.size(-1))
            ).view(batch_size, num_steps, embeddings.size(-1))

            # Combine context info (current node + normalized fuel)
            return torch.cat(
                (current_embed, uav_fuel[:, :, None]), dim=-1
            )

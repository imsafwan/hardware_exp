...
def load_model(path, n_charging_station, epoch=None):
    from nets.attention_model_modified_v3 import AttentionModel
...
def sample_many(inner_func, get_cost_func, input, batch_rep=1, iter_rep=1):
    ...
    for i in range(iter_rep):
        ...
        _log_p, pi,  cost, agent = inner_func(input) #m_t_t
    ...
    #l =  pis[0].clone()[0,:,-1].squeeze(0).shape
    ...
    pis = torch.cat(
        [F.pad(pi, (l, max_length - pi.size(-1))) for pi in pis],
        1

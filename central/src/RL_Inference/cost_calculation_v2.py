...
class ScenarioParameters:
    ...
    def __init__(self, file_path):
        ...
        self.uav_data_points = [(round(n['location']['x'], 6), round(n['location']['y'], 6))
                                for n in nodes_info if n['type'] == 'air_only']
...
class UAV_UGV_Env:
    ...
    def cost_cal(self, actions):
        ...
        uav_action_yaml = {f'sortie_{sortie_n}': [
            {'type': 'takeoff', 'location': uav_position, 'end_time': round(uav_time)}
        ]}
        ugv_action_yaml = {f'sortie_{sortie_n}': [
            {'type': 'allow_takeoff', 'location': ugv_position, 'end_time': round(ugv_time)}
        ]}

        # === Iterate through actions ===
        for j, agent in actions:
            # UAV actions only
            if agent != 1:
                continue
            ...
            # UAV movement
            uav_action_yaml[f'sortie_{sortie_n}'].append(
                {'type': 'move_to_location', 'location': uav_position, 'end_time': round(uav_time)}
            )
            ...
            if j < self.N_ugv:
                ...
                for k in range(1, len(path)):
                    ...
                    ugv_time += step_time
                    ugv_position = path[k]
                    ugv_action_yaml[f'sortie_{sortie_n}'].append(
                        {'type': 'move_to_location', 'location': ugv_position, 'end_time': round(ugv_time)}
                    )

                ugv_action_yaml[f'sortie_{sortie_n}'].append(
                    {'type': 'allow_land_on_UGV', 'location': ugv_position,
                     'end_time': round(ugv_time + self.landing_time)}
                )

                # UAV landing
                uav_time += self.landing_time
                uav_action_yaml[f'sortie_{sortie_n}'].append(
                    {'type': 'land', 'location': uav_position, 'end_time': round(uav_time)}
                )
        ...
        return {
            ...
            "total_mission_time": round(total_time)
        }
...
# ============================================================
def get_scores_for_method(method, scenario_path):
    """Reads actions_{method}.csv and returns sortie breakdowns."""
    df = pd.read_csv(f'./Tours_output/actions_{method}.csv')
    actions_list = df['route']

    results = []
    for ins, route_str in enumerate(actions_list):
        env = UAV_UGV_Env(scenario_path)
        actions = ast.literal_eval(route_str)  # [(index, agent), ...]

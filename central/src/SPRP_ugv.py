...
#---------------------------------------------------------------------------------#  
uav_full_flying_time = 90 # sec = min*60
...
def ugv_needs_replan(uav_state, ugv_state, uav_actions, ugv_actions, road_network):
    ...
    if feasible_relaxed:
        logger.info("found feasible rp under relaxed endurance")
...
def ugv_is_replanning(uav_state, ugv_state, uav_actions, ugv_remaining_actions, road_network):
    ...
    logger.info("Replan status: {}".format(status))
    ...
    if status == "PLAN_END":
        ...
        ugv_plan = [{"type": "allow_land_on_UGV","location": {"lat": uav_loc[0],"lon": uav_loc[1]},"ins_id": "land_1","end_time": uav_time  + land_time}]
        ...
        ugv_plan = ugv_remaining_actions
        logger.info('UAV plan: {} \n'.format(uav_plan))
        logger.info('UGV plan: {} \n'.format(ugv_plan))
        logger.info("No change on UAV/UGV plans.")

    else: # new rendezvous point

        uav_cur_loc = uav_state["loc"]
        uav_next_p = uav_state.get("curr_p", None)
        uav_next_p_time = haversine_distance(uav_cur_loc, uav_next_p) / uav_velocity
        elapsed = uav_state["time_elapsed"] + uav_next_p_time

        # add end time to each action
        for act in uav_plan:
            if act["type"] == "move_to_location":
                tgt = (act["location"]["lat"], act["location"]["lon"])
                act['aoi_id'] =    scene.corrd_to_id_map[tgt]
                dist = haversine_distance(uav_next_p, tgt)
...
def find_new_rp(seq, uav_cur_state, candidate_rendezvous_points):
    uav_cur_loc = uav_cur_state["loc"]
    elapsed = uav_cur_state["time_elapsed"]
    uav_next_p = uav_cur_state.get("curr_p", None)
    uav_next_p_time = haversine_distance(uav_cur_loc, uav_next_p) / uav_velocity
    endurance = uav_flying_time
    remaining_time = endurance - elapsed - uav_next_p_time
    ...
    for act in seq:
        if act["type"] == "move_to_location":
            ...
            raise ValueError("Should not have landing as action")
            #future_actions_time += land_time
...
def compute_candidate_points(uav_state, ugv_state, road_network):
    ...
    uav_next_p = uav_state.get("curr_p", None)
    uav_next_p_time = haversine_distance(uav_loc, uav_next_p) / uav_velocity
    uav_remaining_time = uav_flying_time - uav_elapsed - land_time - uav_next_p_time 
    ...
    for node in road_network.nodes:
        ...
        dist_uav = haversine_distance(uav_next_p, node_coord)
...
def create_ugv_plan(ugv_state, new_rp, road_network):
    ...
    elapsed = ugv_state['time_elapsed']
    # ugv_plan.append({

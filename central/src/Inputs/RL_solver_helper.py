...
def load_scene_yaml(path):
    ...
    for node in nodes:
        ...
        elif node["type"] == "air_only":
            uav_points.append((lat, lon))

    return np.array(ugv_points, dtype=np.float32), np.array(uav_points, dtype=np.float32), connections
...
def scene_to_dataset(scene_path, ref_lat, ref_lon, device="cuda", visualize=True):
    ...
    """
    ugv_gps, uav_gps, connections = load_scene_yaml(scene_path)
